...
def power_iteration(kernel, u_init, maxiter_spectral, unroll=True, jit=True):
...
def spectral_normalization(kernel, u, maxiter_spectral, jit=True, unroll=True):
...
def bjorck_algorithm(kernel, maxiter_bjorck, beta=0.5, order=1, unroll=True, jit=True):
...
def kernel_orthogonalization(kernel,
  ...
  if u is None:
    # use average of columns as initial vector.
    u = jnp.mean(kernel, axis=0)
...
class BjorckDense(nn.Module):
  ...
  @nn.compact
  def __call__(self, inputs, train=None, maxiter_spectral=None, maxiter_bjorck=None):
    """Forward pass.
    ...
    if train:
      ...
      ortho_ker, _u, _sigma = kernel_orthogonalization(kernel, u.value, maxiter_spectral, maxiter_bjorck, unroll=True,
                                                       jit=True)
